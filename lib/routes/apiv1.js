...
function postExec (req, res) {
  ...
  if (commandName === 'multi' || commandName === 'monitor' || commandName === 'hmset') {
    return res.json({data: `ERROR: Command ${commandName} not supported via web cli`});
  }else if(commandName === "hset"){
    // parts[parts.length-1] = myutil.rpc(parts[parts.length-1])
  }
  ...
  args.push(function (err, results) {
      ...
      if(parts[0] === "hgetall"){
        results = myutil.handlerMsgpackObject(results)
      }else if (["hget", "hmget"].includes(parts[0])){
        results = myutil.handlerMsgpack(results)
      }else{
  ...
  connection.callBuffer(commandName, ...args);
}
